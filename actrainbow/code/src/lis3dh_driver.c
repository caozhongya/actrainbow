...
u8_t LIS3DH_ReadReg(u8_t Reg, u8_t* Data) {
    ...
    UCB1I2CSA = LIS3DH_ADDRESS;        // LIS3DH_ADDRESS
    UCB1CTL1 |= UCTR;                 // 写模式
    UCB1CTL1 |= UCTXSTT;              // 发送启动位和写控制字节

    UCB1TXBUF = Reg;                  // 发送字节地址，必须要先填充TXBUF
...
u8_t LIS3DH_MultiReadacc(u8_t *Data ,u8_t count) {
    ...
    UCB1TXBUF = 0x28|0x80;            // 发送字节地址，必须要先填充TXBUF
    // 等待UCTXIFG=1 与UCTXSTT=0 同时变化 等待一个标志位即可
    while(!(UCB1IFG & UCTXIFG))
    {
      if( UCB1IFG & UCNACKIFG )       // 若无应答 UCNACKIFG=1
...
status_t LIS3DH_Click_Init(u8_t thresholdValue, u8_t limitValue,u8_t latencyValue,u8_t windowValue){
 
  
  if(!LIS3DH_SetClickTHS(thresholdValue))//1g
      return MEMS_ERROR;
  if(!LIS3DH_SetClickLIMIT(limitValue))//6*1/50hz=6*20ms=120ms,越大越容易识别
      return MEMS_ERROR;
  if(!LIS3DH_SetClickLATENCY(latencyValue))//32*1/50hz=32*20ms=640ms
      return MEMS_ERROR;
  if(!LIS3DH_SetClickWINDOW(windowValue))//2*1/50hz=32*20ms=640ms
      return MEMS_ERROR;
  
      //开启双击响应,关闭单击响应
  if(!LIS3DH_SetClickCFG( LIS3DH_ZD_ENABLE | LIS3DH_ZS_ENABLE | LIS3DH_YD_ENABLE | 
                          LIS3DH_YS_ENABLE | LIS3DH_XD_ENABLE | LIS3DH_XS_ENABLE ))
      return MEMS_ERROR;
...
status_t LIS3DH_Init(){
  ...
  if(!LIS3DH_SetAxis(LIS3DH_X_ENABLE | LIS3DH_Y_ENABLE | LIS3DH_Z_ENABLE))
      return MEMS_ERROR;
  //Double Click inition.
//  if(!LIS3DH_Click_Init(40,3,32,20))
//     return MEMS_ERROR;
  return MEMS_SUCCESS;
}
...
status_t LIS3DH_Init_SleepMode(){
